...
double calculate(char, double,
                 double);  // calculate the number be inputed with opr
...
int main( ) {
    ...
    while (tolower(pass) == 'n') {
        printf("%s", "\nExcept +-*/ ( ) other symbol will be ignored\nput ur "
                     "question there ( only integer & every item < 2^(31-1) --> ");
...
void print(int controller) {
    LLAPtr current = LLAH[controller].LLAFront;
    if (isEmpty(controller))
        puts("List is empty!");
    else
        while (current != NULL) {
            if (current->opr != Nothing)
                printf(" %c", current->opr);
            else
                printf(" %.3lf", current->num);
...
int pop(char* c_out, double* d_out, int controller) {
    ...
    *c_out                   = beban->opr;
    ...
    if (isEmpty(controller))
        LLAH[controller].LLAFront = NULL;  // debug
    else
        LLAH[controller].LLARear->next = NULL;
    return True;
}
...
int shift(char* c_out, double* d_out, int controller) {
    ...
    *c_out                    = beban->opr;
    ...
    if (isEmpty(controller))
        LLAH[controller].LLARear = NULL;  // debug
    else
        LLAH[controller].LLAFront->pre = NULL;
    return True;
}
...
int wordProcess(char* infix) {
    ...
    while (infix[tok] != '\0') {
        ...
        switch (infix[tok]) {
            ...
            case '*':
            case '/':
                if (((!isdigit(c_last)) && c_last != ')') || isHead) return False;
                push(infix[tok++], 0, Queue4in);
                isHead = False;
                break;
            case '(':
                ...
                break;
            case ')':
                if (!isdigit(c_last) && c_last != ')') return False;
                push(infix[tok++], 0, Queue4in);
                isHead = False;
                break;
            case '1':
            ...
            case '0':
                do {
                    sTmp[0] = infix[tok];
                    if (isdigit(infix[tok])) tmp = tmp * 10 + atoi(sTmp);
...
int isAllowedOpr(char c) {
    char opr[]  = {'+', '-', '*', '/', '(', ')'};
    size_t iter = 0;
    while (iter < sizeof(opr))
        if (c == opr[iter++]) return True;
    return False;
}
    ...
    while (shift(&c_infix, &d_infix, postfix)) {
        switch (c_infix) {
            ...
            case '/':
                ...
                break;
            default: push(Nothing, d_infix, Stack);
        }
    }
    pop(&c_pop1, &i_pop1, Stack);
    return i_pop1;
